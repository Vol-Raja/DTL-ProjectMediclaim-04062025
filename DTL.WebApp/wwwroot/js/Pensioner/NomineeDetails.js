//var tempData = [];
/* Share Details Calculation */
/* Team - VirAj */

$('#Arrear').on("input propertychange", function () {
    var _rowShare = 0;
    var _rowCount = 0;
    var _totalShare = 0;
    var _shareVal = parseInt($('#Arrear').val());

    _rowCount = $('#nomineeTbl tbody tr').length;
    if (_rowCount != null && _rowCount != undefined) {
        //alert("Count" + _rowCount);

        $('#nomineeTbl tr').each(function () {
            _rowShare = (!isNaN($(this).find(".sharePercent").html())) ? parseFloat($(this).find(".sharePercent").html()) : 0;
            //_rowShare = parseFloat($(this).find(".sharePercent").val())
            _totalShare = parseFloat(_totalShare) + parseFloat(_rowShare);
        });
    }

    if (_shareVal != null && _shareVal != undefined) {
        _totalShare;
        if ((_shareVal > 100) || (_totalShare > 100)) {
            alert('Share % cannot be greater than 100%');
            $('#Arrear').val(0);
            return;
        }
        else if (_totalShare < 100) {
            if (_shareVal > (100 - _totalShare)) {
                alert('Share % must be in range 0 and ' + (100 - _totalShare));
                $('#Arrear').val(100 - _totalShare);
                return;
            }
        }
        else {
            alert('Total Share % cannot be greater than 100%');
            $('#Arrear').val(0);
        }
    }
    else {
...
$('#updateNomineeTbl').on('click', function () {
    if ($("#NomineeDetails").valid()) {
        ...
        $('#nomineeTbl tbody tr').eq($('#hfRowIndex').val()).find('td').eq(8).html(data.arrear);
