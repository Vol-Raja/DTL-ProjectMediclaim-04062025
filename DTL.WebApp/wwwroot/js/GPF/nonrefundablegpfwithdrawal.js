$(document).ready(function () {
    ...
    $('#submit_form').click(function () {
        ...
        if (ValidateSubmit()) {
            ...
            $('#lblErrorMessage').text('Please provide fields whcih are highlighted in red.');
    ...
    var saveData = function () {
        ...
        $.ajax({
            type: 'POST',
            url: "/GPF/Withdrawal/Save/NonRefundableGPF",
            ...
            success: function (response, status, xhr) {
                if (xhr.status == '200') {
                    ...
                    setTimeout(function () {
                        window.location.href = '/GPF/Withdrawal/NonRefundableList';
                    }, 2000);
                }
            },
    ...
    $('#btnViewNonRefundableGPFApplication').click(function () {
        window.open(window.location.href = '/GPF/Withdrawal/NonRefundableGPF/' + $('#hdnWithdrawalId').val(), '_blank');
    });
    ...
    var PopulateProperties = function () {
        ...
        RemainingContribution = $('#txtRemainingContribution').val();
    ...
    var FileUpload = function (withdrawId) {
        ...
        if (salaryslipfiles.length > 0 || idcardfile != undefined) {
            ...
            $.ajax({
                type: 'POST',
                url: '/GPF/Withdrawal/Upload/nonrefundable/' + withdrawId + '/' + _indicator,
                data: form,
                contentType: 'application/json; charset=utf-8',
                timeout: 0,
                processData: false,
    ...
    var LoadGPFWithdrawalData = function (id) {
        $.ajax({
            type: 'GET',
            url: '/Get/GPF/Modify/nonrefundable/' + $('#hdnWithdrawalId').val(),
            data: "",
            contentType: 'application/json; charset=utf-8',
            //dataType: 'json',
            success: function (response, status, xhr) {
                ...
                $('#txtRemainingContribution').val(response.remainingContribution);
                //};
